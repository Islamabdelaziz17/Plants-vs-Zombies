...
public class Level {
    ...
    ImageView bar;
    @FXML
    ImageView peashooter_menu;
    @FXML
    ImageView sunflower_menu;
    @FXML
    ImageView walnut_menu;
    @FXML
    ImageView cherrybomb_menu;
    @FXML
    GridPane gridPane;
    @FXML
    TextField sun_counter;
    @FXML
    ImageView peashooter_lock;
    @FXML
    ImageView sunflower_lock;
    @FXML
    ImageView walnut_lock;
    @FXML
    ImageView cherrybomb_lock;
    @FXML
    TextField lost;
    Button next_level;
    ...
    private ArrayList<ImageView> sun_image_list;
    private HashMap<ImageView, SunToken> sun_map;
    
    private Random r;
    private Sun_Counter sun_c=new Sun_Counter();
    
    private ArrayList<Lawnmower> lawnmower_list=new ArrayList<>();
    private ArrayList<ImageView> lawnmower_image_list;
    private HashMap<ImageView, Lawnmower> lawnmower_map;
    
    private int[][] trace=new int[9][5];
    private long curr_time;
    
    private ArrayList<Bullet> bullet_list;
    private ArrayList<ImageView> bullet_image_list;
    private HashMap<ImageView, Bullet> bullet_map;

    private ArrayList<Shooter> shooter_list;
    private ArrayList<ImageView> shooter_image_list;
    private HashMap<ImageView, Shooter> shooter_map;
   
    private ArrayList<Zombie> zombie_list;
    private ArrayList<ImageView> zombie_image_list;
    private HashMap<ImageView, Zombie> zombie_map;
    ...
    private ArrayList<ImageView> sunproducer_image_list;
    private HashMap<ImageView, SunTokenProducer> sunproducer_map;
    private ArrayList<Barrier> barrier_list;
    private ArrayList<ImageView> barrier_image_list;
    private HashMap<ImageView, Barrier> barrier_map;

    private ArrayList<Bomb> bomb_list;
    private ArrayList<ImageView> bomb_image_list;
    private HashMap<ImageView, Bomb> bomb_map;

    private ArrayList<Plant> plant_list;
    private ArrayList<ImageView> plant_image_list;
    private HashMap<ImageView, Plant> plant_map;
    ...
    public void set_root(Parent root){}
    ...
    private void add_sun_from_sky() throws FileNotFoundException {
        ...
        SunToken sun = new SunToken(x1, y1,limit);
        sun_list.add(sun);
        
        sun.sun_image.setLayoutX(x1);
        sun.sun_image.setLayoutY(y1);
        sun.sun_image.setFitWidth(75);
        sun.sun_image.setFitHeight(75);
        sun_map.put(sun.sun_image, sun);
        sun.setImview(sun.sun_image);
        sun.sun_image.addEventHandler(MouseEvent.MOUSE_CLICKED, e->{
                if(menu_click){
                    double x=e.getX()-225+sun.sun_image.getLayoutX();
                    double y=e.getY()-70+sun.sun_image.getLayoutY();
                    helper(x,y);
                }
                else {
                    sun_map.get(sun.sun_image).setDead(true);
                    e.consume();
                }
        });
        sun_image_list.add(sun.sun_image);
        anchor_pane.getChildren().add(sun.sun_image);
    ...
    private void add_zombie(){
        ...
        row=r.nextInt(5);
        y1=97*(row+1)-30;
        Zombie new_zombie= new Zombie(y1,30+level*5,90 + level*10,System.nanoTime()-10000000000L, row);
        zombie_list.add(new_zombie);
        new_zombie.zombie_image.setLayoutX(new_zombie.getX());
        new_zombie.zombie_image.setLayoutY(y1);
        new_zombie.zombie_image.setFitWidth(100);
        new_zombie.zombie_image.setFitHeight(100);
        zombie_map.put(new_zombie.zombie_image, new_zombie);
        zombie_image_list.add(new_zombie.zombie_image);
        anchor_pane.getChildren().add(new_zombie.zombie_image);
    }
    ...
    private AnimationTimer timer;    // animation timer class object
    private double elapsed_time;    //elapsed time of the game  level is initialized in level() constructor by 0
     ...
     void gameloop(Stage primaryStage){
        ...
        timer = new AnimationTimer(){
            ...
            public void handle(long now){
                ...
                ts_walnut+=tempp;
                last_update=now;
                bar.setLayoutX(715-(100/55)*elapsed_time);
                if(elapsed_time>=90.0){
                    ...
                    l=s.getController();
                    l.set_window(window2);
                    l.set_level(level_number);
                    l.set_root(root);
                ...
                if(time_since_zombie_last_added>20-level*1){
